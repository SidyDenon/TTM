...
const FAQS = [
  {
    q: "Comment se passe une intervention TTM ?",
    a: `Depuis l’application, vous renseignez votre adresse, le type de panne
(ex: remorquage, crevaison, erreur de carburant) et ajoutez une photo si besoin.
Nous géolocalisons votre position, affectons l’opérateur le plus proche
et vous suivez son arrivée en temps réel.`,
  },
  {
    q: "Quels sont les délais moyens en ville ?",
    a: `En zone urbaine, nous visons 20 à 40 minutes selon le trafic et la disponibilité.
Sur les axes interurbains, le délai dépend de la distance et des conditions.`,
  },
  {
    q: "Comment sont calculés les tarifs ?",
    a: `Tarifs affichés "à partir de", avec transparence avant intervention.
Les coûts varient selon la distance (remorquage), la prestation (batterie, crevaison),
et les horaires (nuit/jours fériés le cas échéant).`,
  },
  {
    q: "Puis-je payer sur place ?",
    a: `Oui : mobile money, espèces ou lien de paiement sécurisé (selon zone).
Une confirmation de paiement et un reçu sont immédiatement envoyés
dans l’application ou par WhatsApp/email.`,
  },
  {
    q: "Puis-je tout gérer depuis l’application ?",
    a: `Oui. De la demande jusqu’au paiement, tout se fait dans l’application :
choix de l’intervention, suivi en temps réel et confirmation finale.`,
  },
  {
    q: "Mon véhicule est bloqué/accidenté, pouvez-vous gérer ?",
    a: `Oui. Nos dépanneuses sont équipées (treuil, sangles, cales). Nous assurons la
sécurisation de la zone, la charge du véhicule et l’acheminement au garage choisi.`,
  },
];
...
function normalizePhoneForWa(phoneRaw) {
  ...
  if (p.startsWith("00")) p = p.slice(2); // wa.me n'accepte pas le préfixe 00
  return p; // ex: 22373585046
}

function buildWhatsAppContactUrl() {
  const phone = normalizePhoneForWa(WHATSAPP_NUMBER);
  const text =
    "Bonjour 👋, j’ai une question à propos de vos services TTM.";
          ...
          <div className="max-w-6xl mx-auto px-4 py-12 grid grid-cols-1 text-center md:grid-cols-3 gap-8">
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              <div className="mt-4 flex gap-3">
                <a href="mailto:support@ttmali.com" className="hover:text-[#800E08]">
